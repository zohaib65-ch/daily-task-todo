      ...
      <CardContent>
        <ul className="space-y-2">
          {priorities.map((priority) => (
            <li key={priority.id} className="flex items-center space-x-2 p-2 rounded bg-muted ">
              <span className="w-6 h-6 flex items-center justify-center bg-muted rounded-full">{priority.id}</span>
              <span>{priority.text}</span>
            </li>
          ))}
          <li className="flex items-center space-x-2 p-2 rounded bg-muted ">
            <input
              type="text"
              value={newPriority}
