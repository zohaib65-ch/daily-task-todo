  ...
  persist(
    (set, get) => ({
      ...
      completeSession: () => {
        ...
        const sessionCount = wasBreak
          ? currentSession.sessionCount
          : currentSession.sessionCount + 1;
        const isLongBreakNext =
          !wasBreak && sessionCount % timerSettings.longBreakInterval === 0;
      ...
      fetchTimerSettings: async () => {
        try {
          const response = await axios.get<TimerSettings>(
            `${API_URL}/users/timer-settings`
          );
          if (response.data) {
      ...
      fetchTimerStats: async () => {
        try {
          const response = await axios.get<TimerStats>(
            `${API_URL}/timer/stats`
          );
